...
| **失败不阻塞** | 注入失败仅记录 error 日志，不阻塞主窗口显示，不影响真实捕获 |
...
- **验证方式**：
  - XCTest：注入示例数据后，调用 `SearchService.search(query: "报错", limit: 5)`，断言结果中至少 1 条 `contentType == .error`；同理验证"代码"→`.code`、"链接"→`.link`
...
- UI-SD-01 预置方式：测试 setup 中直接调用 `SampleDataSeeder.seedIfNeeded(store:embeddingService:)` 写入临时数据库（或通过 `--UITEST_RESET_ONBOARDING` 触发首启注入流程）
- UI-SD-02/03 预置方式：测试 setup 中通过 EncryptedStore 直接写入预置数据（13 条 isSample=true + 2 条 isSample=false），使用 `--UITEST_SHOW_MAIN_WINDOW` 跳过引导
