...
| F1.2 | 自动分类（12 种类型：11 入库 + 1 敏感） | AI 自动识别内容类型，11 种入库类型准确率 ≥ 80%，敏感内容识别后不入库 |
...
├── Fixtures/
│   ├── classification_samples.json      # 220 条分类测试样本（11 种入库类型 × 20 条）
...
|--------|------|---------|---------|---------|
| 分类测试集（`classification_samples.json`） | 220 条（11 种入库类型 × 20 条） | Phase 1 开始前 | 人工标注正确类型（ContentType） | 双人交叉抽查 10% 样本，分歧协商解决 |
| 搜索测试集（`search_queries.json`） | 50 个查询 | Phase 1 开始前 | 人工标注期望返回的 Top-5 条目 | 双人交叉抽查 10% 查询，验证标注一致性 |
| 敏感内容样本（`sensitive_samples.json`） | 6 种模式，共 20 条 | Phase 1 开始前 | 人工标注敏感类型 | 覆盖正则边界用例 |
...
**范围**：
- F1.2 自动分类（11 种入库类型，准确率 ≥ 80%；敏感内容由 SensitiveDetector 识别）
